# =====================================================================
# virtual_sequence_modify.tcl
if {$gm_type eq "RTL"} {
    # =====================================================================
    # virtual_sequence_modify.tcl
    # Reads sequence_list from run.tcl (already in scope via source)
    # Replaces the single "my" placeholder in templete_virtual_sequence with one
    ...
    set virtual_sequence_file   "${output_dir}/sequences/virtual/${project_name}_virtual_sequence.sv"
    
    if {![file exists $virtual_sequence_file]} {
        puts "Error: virtual_sequence file not found at $virtual_sequence_file"
        exit 1
    }
    
    # --------- Read generated virtual_sequence file ---------
    set fp   [open $virtual_sequence_file r]
    set data [read $fp]
    close $fp
    ...
    foreach seq_info $sequence_list {
        ...
        append create_block \
            "       ${var_pad} =    ${handle_pad} type_id:: create(${create_str_pad});\n"
    }
    ...
    foreach seq_info $sequence_list {
        ...
        append run_block \
            ...
            "        ${var_str}.start(p_sequencer.sequencer);     \n" \
    ...
    set new_data [regsub \
        "my_seq\\s+=\\s+${project_name}_my_sequence::\\s+type_id:: create\\(\"my_seq\"\\s*\\);" \
        $new_data \
        $create_block]
    ...
    # Write back
    # =====================================================================
    set fp [open $virtual_sequence_file w]
    puts -nonewline $fp $new_data
    close $fp
    
    puts "virtual_sequence file updated successfully: $virtual_sequence_file"
} else {
    # =====================================================================
    # virtual_sequence_modify.tcl
    # =====================================================================
    ...
    foreach seq_info $expanded_seqs {
        ...
        set handle_pad [rpad $handle_str $max_handle_len]
        set create_str_pad [rpad $create_str $max_create_str_len]
        append create_block "       ${var_pad} =    ${handle_pad} type_id:: create(${create_str_pad});\n"
    }
    ...
    foreach seq_info $sequence_list {
        ...
        if {$single_agent} {
            ...
            append run_block "        ${var_str}.start(p_sequencer.sequencer);     \n"
            ...
            for {set i 1} {$i <= $Num_AC_agent} {incr i} {
                ...
                append run_block "           ${var_str}.start(p_sequencer.${aname}sequencer);\n"
            }
    ...
    set new_data [regsub "my_seq\\s+=\\s+${project_name}_my_sequence::\\s+type_id:: create\\(\"my_seq\"\\s*\\);" $new_data $create_block]
